import{f as G,a as q,b as J,c as X,d as Z}from"./assets/tastyTreatsApi-D_6eBPKE.js";const y=document.querySelector("#recipes-container"),A=document.querySelector("#pagination"),B=document.querySelector("#category-list"),b=document.querySelector(".categories-box");let u=1,O=1,v=null,x=N();function N(){const e=window.innerWidth;return e<768?6:e<1280?8:9}async function P(){try{const t=(await G()).map(r=>`
        <li>
          <button 
        ...
        </li>`).join("");B.innerHTML=t;const n=B.querySelectorAll(".category-btn");n.forEach(r=>{r.addEventListener("click",()=>{const s=r.dataset.category;s!==v&&(v=s,u=1,n.forEach(i=>i.classList.remove("active")),b.classList.remove("active"),r.classList.add("active"),E(u))})})}catch(e){console.error("Failed to load categories:",e)}}b&&b.addEventListener("click",()=>{v!==null&&(v=null,u=1,B.querySelectorAll(".category-btn").forEach(t=>t.classList.remove("active")),b.classList.add("active"),E(u))});async function E(e=1){try{x=N();const t=await q({page:e,limit:x,category:v||void 0}),n=t.results;if(O=t.totalPages||Math.ceil(t.totalResults/x),!n||n.length===0){y.innerHTML="<p>No recipes found to display.</p>",A.innerHTML="";return}const r=n.map(s=>{const i=s.rating/5*100;return`
  ...
  `,d.innerHTML=e<=1?"":i}function de(e){if(!o.recipes)return;if(!(e!=null&&e.length)){o.recipes.innerHTML="";return}const t=e.map(n=>{const{_id:r,title:s="Untitled",description:i="",rating:a=0,preview:f,thumb:p}=n,m=f||p||"",M=h(s),T=h(i),z=Number(a)||0,Y=Math.max(0,Math.min(100,z/5*100));return`
      <div class="recipe-card" data-id="${r||""}">
        ...
        ${m?`<img class="recipe-card-image" src="${m}" alt="${M}" loading="lazy">`:""}
      ...
      </div>`}).join("");o.recipes.innerHTML=t}async function k(){try{if(!oe()){le();return}ae(),D(!0);const{time:e,area:t,ingredient:n,page:r,limit:s}=l,i=await se(q({time:e,area:t,ingredient:n,page:r,limit:s})),a=Array.isArray(i==null?void 0:i.results)?i.results:Array.isArray(i)?i:Array.isArray(i==null?void 0:i.recipes)?i.recipes:[],f=l.query.toLowerCase(),p=f?a.filter(m=>String(m.title||"").toLowerCase().includes(f)):a;if(H){if(de(p),d){const m=Number(i==null?void 0:i.totalPages)||Number(i==null?void 0:i.pageCount)||0,M=Number(i==null?void 0:i.total)||Number(i==null?void 0:i.count)||Number(i==null?void 0:i.resultsCount)||Number(i==null?void 0:i.hits)||(Array.isArray(i==null?void 0:i.results)?i.results.length:0),T=m||Math.max(1,Math.ceil((M||p.length)/(s||6)));ce(T,l.page)}!p.length&&o.recipes&&(o.recipes.innerHTML='<p class="no-results">No recipes found</p>')}document.dispatchEvent(new CustomEvent("recipes:filtered",{detail:{recipes:p,paging:{page:r,limit:s},raw:i}}))}catch(e){console.error("[filters] applyFilters error:",(e==null?void 0:e.message)||e),H&&o.recipes&&(o.recipes.innerHTML=(e==null?void 0:e.message)==="REQUEST_TIMEOUT"?'<p class="no-results">İstek zaman aşımına uğradı. Lütfen tekrar deneyin.</p>':'<p class="no-results">Bir şeyler ters gitti. Lütfen tekrar deneyin.</p>'),document.dispatchEvent(new CustomEvent("recipes:filtered",{detail:{recipes:[],error:!0,reason:(e==null?void 0:e.message)||String(e)}}))}finally{D(!1)}}async function pe(){w("time",ne.map(e=>({value:re(e),label:e})));try{const e=await X(),t=(Array.isArray(e)?e:[]).map(n=>typeof n=="string"?n:n==null?void 0:n.name).filter(Boolean).map(n=>({value:n,label:n}));w("area",t)}catch(e){console.error("Areas yüklenemedi",e),w("area",[])}try{const e=await Z(),t=(Array.isArray(e)?e:[]).filter(n=>(n==null?void 0:n._id)&&(n==null?void 0:n.name)).map(n=>({value:n._id,label:n.name}));w("ingredient",t)}catch(e){console.error("Ingredients yüklenemedi",e),w("ingredient",[])}}function w(e,t){const n=o.dropdowns.find(i=>i.dataset.type===e);if(!n)return;const r=n.querySelector(".dropdown-menu"),s=n.querySelector(".dropdown-toggle");!r||!s||(r.innerHTML=(t||[]).map(i=>`<li><button type="button" data-value="${h(i.value)}" data-label="${h(i.label)}">${h(i.label)}</button></li>`).join(""),s.addEventListener("click",()=>{const i=!n.classList.contains("open");_(),n.classList.toggle("open",i)}),r.addEventListener("click",i=>{const a=i.target.closest("button[data-value]");if(!a)return;const f=a.getAttribute("data-value")??"",p=a.getAttribute("data-label")??a.textContent.trim();e==="time"?l.time=f:e==="area"?l.area=p:e==="ingredient"&&(l.ingredient=f,l.ingredientLabel=p),s.textContent=p,n.classList.remove("open"),l.page=1,k()}))}document.addEventListener("click",e=>{const t=e.target.closest(".custom-dropdown");if(!t)return _();o.dropdowns.forEach(n=>{n!==t&&n.classList.remove("open")})});document.addEventListener("keydown",e=>{e.key==="Escape"&&_()});function ue(){if(!o.search||!o.searchBtn)return;const e=()=>{l.query=o.search.value.trim(),l.page=1,k()};o.searchBtn.addEventListener("click",e),o.search.addEventListener("keydown",t=>{t.key==="Enter"&&(t.preventDefault(),e())})}function ge(){o.resetBtn&&o.resetBtn.addEventListener("click",()=>{l.query="",l.time="",l.area="",l.ingredient="",l.ingredientLabel="",l.page=1,o.search&&(o.search.value=""),k()})}(async function(){try{o.recipes&&(o.recipes.classList.add("recipes-list"),o.recipes.style.display="none"),d&&(d.style.display="none"),await pe(),ue(),ge(),await k(),d&&d.addEventListener("click",t=>{const n=t.target.closest(".pagination-btn");if(!n||n.classList.contains("dots"))return;const r=parseInt(n.dataset.page,10);!isNaN(r)&&r!==l.page&&(l.page=r,k())}),document.addEventListener("click",t=>{const n=t.target.closest(".heart-btn");n&&n.classList.toggle("active")}),document.addEventListener("click",t=>{const n=t.target.closest(".recipe-card-button");if(n){const r=n.closest(".recipe-card"),s=r==null?void 0:r.dataset.id;s&&console.log("Recipe clicked:",s)}})}catch(t){console.error("[filters] init error:",t),o.recipes&&(o.recipes.innerHTML='<p class="no-results">Bir şeyler ters gitti. Lütfen tekrar deneyin.</p>')}})();const Q="favoriteRecipes";function W(){try{return JSON.parse(localStorage.getItem(Q))||[]}catch{return[]}}function U(e){localStorage.setItem(Q,JSON.stringify(e))}function fe(e){const t=W();if(t.some(r=>r._id===e._id)){const r=t.filter(s=>s._id!==e._id);U(r)}else t.push(e),U(t)}function ve(e){return W().some(n=>n._id===e)}const R=document.querySelector("#recipes-container"),j=document.querySelector("#pagination");let L=[],$=1,V=1,C=F();function me(e){const t=e.rating/5*100,n=ve(e._id)?"active":"";return`
